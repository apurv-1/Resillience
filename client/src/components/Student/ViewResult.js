...
const useStyles = makeStyles((theme) => ({
  ...
  spanScore: {
    ...
    height: "8rem",
    background: "rgba( 74, 74, 74, 0.05 )",
    boxShadow: "0 6px 12px 0 rgba( 31, 38, 135, 0.37 ), inset 0 6px 2px 0  rgba( 13, 113, 230, 0.80 )",
    backdropFilter: "blur( 20.0px )",
    WebkitBackdropFilter: "blur( 20.0px )",
    borderRadius: "12px",
    "@media only screen and (max-width: 1125px)": {
    ...
    for (let index = 0; index < questions.length; index++) {
      if (selectedAnswers[index]) {
        questions[index].correctOption === selectedAnswers[index]
          ? (correct = correct + 1) && (timeSpentOnCorrect = timeSpentOnCorrect + timeElapsed[index]) && timeElapsed[index] <= timePerQuestion
  ...
  return (
    <div className={classes.root}>
      ...
      <div className={classes.scoreContainer}>
        ...
        <span className={classes.span}>
          <h2>
            {timeSpentOnCorrect > 60 ? `${parseInt(timeSpentOnCorrect / 60)}𝗺 ${timeSpentOnCorrect % 60}` : timeSpentOnCorrect}
            𝘀
          </h2>
        ...
        <span className={classes.span}>
          <h2>
            {timeSpentOnInCorrect > 60 ? `${parseInt(timeSpentOnInCorrect / 60)}𝗺 ${timeSpentOnInCorrect % 60}` : timeSpentOnInCorrect}
            𝘀
          </h2>
        ...
        <span className={classes.span}>
          <h2>
            {timeSpentOnNotAttempted > 60 ? `${parseInt(timeSpentOnNotAttempted / 60)}𝗺 ${timeSpentOnNotAttempted % 60}` : timeSpentOnNotAttempted}
            𝘀
          </h2>
      ...
      <div className={classes.container}>
        <div className={classes.analysisBlock}>
          ...
          <FormControl className={classes.analysisDropdown}>
            <Select labelId="controlled-open-select-label" variant="outlined" open={open} onClose={() => setOpen(false)} onOpen={() => setOpen(true)} value={type} onChange={handleTestType} fullWidth>
        ...
        <div className={classes.analysisBlock}>
          ...
          <FormControl className={classes.analysisDropdown}>
            ...
            >
              ...
              {test.testType === "pcb" ? <MenuItem value={"Biology"}>Biology</MenuItem> : <MenuItem value={"Maths"}>Maths</MenuItem>}
            </Select>
          </FormControl>
        </div>
        <div className={classes.subjectScore}>
          ...
          <span className={classes.span}>
            <h3>
              {SubjectwiseTime > 60 ? `${parseInt(SubjectwiseTime / 60)}𝗺 ${SubjectwiseTime % 60}` : SubjectwiseTime}
              𝘀
            </h3>
      ...
      {type === "correct" ? (
        <CorrectQuestionsComponent cSubject={cSubject} selectedAnswers={selectedAnswers} timeElapsed={timeElapsed} test={test} questions={questions} />
      ) : type === "incorrect" ? (
        <IncorrectQuestionsComponent cSubject={cSubject} selectedAnswers={selectedAnswers} timeElapsed={timeElapsed} test={test} questions={questions} />
      ) : type === "notAnswered" ? (
        <NotAnsweredQuestionsComponent cSubject={cSubject} selectedAnswers={selectedAnswers} timeElapsed={timeElapsed} test={test} questions={questions} />
      ) : (
        type === "all" && <AllQuestionsComponent cSubject={cSubject} selectedAnswers={selectedAnswers} timeElapsed={timeElapsed} test={test} questions={questions} />
      )}
    </div>
  );
};
