...
router.post("/student-signup", (req, res) => {
  const { name, email, password, batch, contact, parentContact, fname, address, picture, attemptedTests } = req.body;
  if (!name) {
  ...
  Student.findOne({ email }).then((savedStudent) => {
    ...
    bcrypt.hash(password, 12).then((encryptedPassword) => {
      const student = new Student({ name, email, password: encryptedPassword, batch, contact, parentContact, fname, address, picture, attemptedTests });
    ...
    bcrypt
      ...
      .then((doMatch) => {
        if (doMatch) {
          ...
          const { _id, name, email, batch, contact, parentContact, fname, address, picture } = savedStudent;
          res.json({
            token,
            student: {
              _id,
      ...
      student.save().then((mail) => {
        transporter.sendMail({
          ...
          html: `<h2>Hey! ${email},</h2>
					...
					Click <a href="https://resillience.in/reset-password/${token}" >here</a> 
